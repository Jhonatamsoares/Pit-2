...
import com.cs5500.Delivery.exception.CommentAlreadyExistException;
import com.cs5500.Delivery.exception.OrderAlreadyCheckoutException;
...
public class OrderController {
  ...
  public int deleteOrder(@PathVariable("id") String id)
    ...
    if (res == 0) {
      throw new OrderNotExistException("Pedido não existe");
    }
    if (res == -1) {
      throw new OrderAlreadyDeliverException(
          "Não é possível cancelar o pedido. Está na entrega ou pronto");
    }
    return res;
  }
  ...
  public int checkoutUsers(@RequestBody String jsonOrders)
    ...
    if (res == -1) {
      throw new OrderAlreadyCheckoutException("Pedido já finalizado");
    }
    return res;
  }
  ...
  public int addComment(@RequestBody String jsonOrder)
    ...
    if (res == 0) throw new CommentAlreadyExistException("Cada pedido só pode ter um comentário");
    if (res == -1) throw new OrderNotExistException("O pedido não existe");
    return res;
  }
